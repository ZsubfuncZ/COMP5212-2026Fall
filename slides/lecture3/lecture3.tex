...
\begin{frame}
  \centering
  \vspace*{0.70cm}
  {\fontsize{33}{39}\selectfont\bfseries Rademacher Complexity and VC Dimension\par}
  \vspace{0.34cm}
  {\fontsize{16}{20}\selectfont COMP 5212 -- Machine Learning\par}
  \vspace{0.14cm}
  {\fontsize{15}{19}\selectfont Fall 2026\par}
  \vspace{0.72cm}
  {\fontsize{14}{18}\selectfont
  Based on Chapter 3 of \emph{Foundations of Machine Learning}, 2nd ed.\par}
  \vfill
  {\small Mehryar Mohri, Afshin Rostamizadeh, and Ameet Talwalkar\par}
\end{frame}

% 2 ---------------------------------------------------------------------------
\begin{frame}[t]{Where This Lecture Fits}
  \vspace{0.06cm}
  \begin{center}
  \begin{tikzpicture}[
      box/.style={draw=black,rounded corners=4pt,minimum width=3.25cm,minimum height=1.08cm,align=center,fill=softgray},
      focus/.style={draw=deepblue,very thick,rounded corners=4pt,minimum width=3.45cm,minimum height=1.18cm,align=center,fill=softblue},
      arr/.style={-{Latex[length=3mm]},very thick,deepblue},
      node distance=0.72cm
    ]
    \node[box] (finite) {Finite hypothesis sets\\union bound};
    \node[focus,right=of finite] (infinite) {Infinite hypothesis sets\\capacity measures};
    \node[box,right=of infinite] (model) {Model selection, margins,\\kernels, boosting};
    \draw[arr] (finite) -- (infinite);
    \draw[arr] (infinite) -- (model);
  \end{tikzpicture}
  \end{center}
  \vspace{0.20cm}
  \begin{itemize}
    \setlength{\itemsep}{0.13cm}
    \item Lecture 2 handled finite $\Hc$ using $\log\abs{\Hc}$.
    \item Lecture 3 replaces cardinality by notions that remain useful when $\abs{\Hc}=\infty$.
    \item The main tools are \textbf{Rademacher complexity}, the \textbf{growth function}, and \textbf{VC dimension}.
  \end{itemize}
  \SourceNote{Core chapter organization follows Mohri, Rostamizadeh, and Talwalkar (2018), Chapter 3.}
\end{frame}
...
\begin{frame}[t]{Learning Goals}
  ...
  \begin{itemize}
    ...
    \item use Sauer's lemma to derive distribution-free generalization guarantees;
    \item interpret lower bounds in the realizable and agnostic settings.
  \end{itemize}}
\end{frame}


\begin{frame}[t]{Motivation: Infinite Hypothesis Sets}
  \vspace{0.04cm}
...
% 8 ---------------------------------------------------------------------------
\SectionSlide{Part I: Rademacher Complexity}{A data-dependent measure of the ability to fit random noise}
...
% 30 --------------------------------------------------------------------------
\begin{frame}[t]{Checkpoint I}
  \vspace{0.04cm}
  Discuss briefly with a neighbor:
  \begin{enumerate}
    \setlength{\itemsep}{0.20cm}
    \item Why is the supremum \emph{inside} the expectation over signs?
    \item What happens if we use only one fixed hypothesis instead of a class?
    \item Why does a class realizing every labeling of $m$ points have complexity $1$?
    \item Which proof step uses boundedness of the loss in $[0,1]$?
  \end{enumerate}
  \vspace{0.10cm}
...
% 32 --------------------------------------------------------------------------
\SectionSlide{Part II: Growth Functions and VC Dimension}{Turn an infinite class into finitely many sample-level behaviors}
...
\begin{frame}[t]{Massart's Lemma: Exponential-Moment Step}
  ...
  \begin{align*}
    ...
    &\le \E_\sigma\exp\!\left(\lambda\sup_{a\in A}\angles{\sigma,a}\right)\\
    &\le \sum_{a\in A}\E_\sigma e^{\lambda\angles{\sigma,a}}.
  \end{align*}
  ...
  \begin{align*}
    ...
    &\le \exp\!\left(\frac{\lambda^2r^2}{2}\right).
  \end{align*}}
\end{frame}
...
\begin{frame}[t]{Massart's Lemma: Optimize the Parameter}
  ...
  Choose
  \[
    \lambda=\frac{\sqrt{2\log\abs{A}}}{r}
  \]
...
% 42 --------------------------------------------------------------------------
\SectionSlide{VC Dimension}{The largest sample on which a class realizes every binary labeling}
...
\begin{frame}[t]{Shattering and VC Dimension}
  ...
  \begin{beamercolorbox}[rounded=true,sep=0.20cm,wd=\linewidth]{block body}
    \[
      \boxed{
      \vcdim(\Hc)=\max\{m:\PiH(m)=2^m\}.}
    \]
  \end{beamercolorbox}
  \vspace{0.15cm}
  \begin{itemize}
    \setlength{\itemsep}{0.10cm}
    \item It is a purely combinatorial notion for binary-valued classes.
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item Upper bound: among any five points, one is not an $x$- or $y$-extreme.
        \item Label that point negative and the four extremes positive.
        \item A rectangle containing all extremes must contain the fifth point.
      \end{itemize}}
    \end{column}
  \end{columns}
  \vspace{0.01cm}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}
        ...
        \item If one point lies inside the triangle of the other three, label the interior point positive and the outer points negative.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.04cm}
...
\begin{frame}[t]{Finite and Infinite VC Dimension}
  ...
  \begin{itemize}
    \setlength{\itemsep}{0.10cm}
    \item Convex polygons with at most $d$ sides have $\vcdim=2d+1$.
  ...
  \begin{tikzpicture}[scale=0.57]
    ...
    \node[align=left,text width=5.5cm] at (7.0,0.4) {More allowed sides permit more alternating label patterns on a circle.};
  \end{tikzpicture}
  \end{center}
  \vspace{0.02cm}
...
% 51 --------------------------------------------------------------------------
\begin{frame}[t]{Checkpoint II: Compute a VC Dimension}
  \vspace{0.02cm}
  Consider unions of at most $k$ disjoint intervals on the real line:
  \[
    h(x)=1\quad\Longleftrightarrow\quad
    x\in\bigcup_{j=1}^{k}[a_j,b_j].
  \]
  \begin{enumerate}
    \setlength{\itemsep}{0.15cm}
    \item Propose a configuration giving a lower bound.
    \item Identify a labeling yielding an upper bound.
    \item What VC dimension do you conjecture?
  \end{enumerate}
  \vspace{0.08cm}
  \KeyBox{Answer: $2k$. A labeling on $2k$ ordered points has at most $k$ positive runs; the alternating labeling on $2k+1$ points has $k+1$.}
\end{frame}
...
% 53 --------------------------------------------------------------------------
\SectionSlide{Part III: Sauer's Lemma and Learning Bounds}{Finite VC dimension forces polynomial growth}
...
\begin{frame}[t]{Sauer's Lemma}
  ...
  \begin{itemize}
    ...
    \item For $m>d$, it is much smaller than $2^m$.
  \end{itemize}
...
\begin{frame}[t]{VC Generalization Bound via Rademacher Complexity}
  \vspace{0.02cm}
  Let $\Hc\subseteq\{h:\X\to\pmone\}$ have VC dimension $d$, with $m\ge d$. Combining Sauer's lemma with the Rademacher bound gives, with probability at least $1-\delta$, for all $h\in\Hc$,
  \begin{beamercolorbox}[rounded=true,sep=0.18cm,wd=\linewidth]{block body}
    \[
      \boxed{
      R(h)\le\Rhat(h)
...
\begin{frame}[t]{The Standard VC Bound}
  \vspace{0.02cm}
  A direct symmetrization and growth-function argument gives another classical form. With probability at least $1-\delta$, for all $h\in\Hc$,
  \begin{beamercolorbox}[rounded=true,sep=0.18cm,wd=\linewidth]{block body}
    \[
      \boxed{
      R(h)\le\Rhat(h)
...
\begin{frame}[t]{Worked Example: Linear Separators}
  ...
  \begin{itemize}
    \setlength{\itemsep}{0.12cm}
    \item Capacity grows linearly with ambient dimension.
    \item The bound ignores margin information and observed-sample geometry.
    \item Margin bounds in the SVM chapter can be much sharper than raw VC dimension.
  \end{itemize}
  \vspace{0.06cm}
...
% 66 --------------------------------------------------------------------------
\SectionSlide{Lower Bounds}{Why the dependence on VC dimension cannot be removed}

% 67 --------------------------------------------------------------------------
\begin{frame}[t]{Lower Bound: Realizable Case}
  \vspace{0.01cm}
  Let $\Hc$ have VC dimension $d>1$. For any learning algorithm $A$, there exist a distribution $\D$ and target $f\in\Hc$ such that
  \begin{beamercolorbox}[rounded=true,sep=0.18cm,wd=\linewidth]{alertblock body}
    \[
      \boxed{
      \Prb_{S\sim\D^m}\!\left[
...
% 69 --------------------------------------------------------------------------
\begin{frame}[t]{Realizable Lower Bound: Proof Sketch}
  \vspace{0.01cm}
  \begin{enumerate}
    \setlength{\itemsep}{0.12cm}
    \item Choose rare-point mass so the sample sees at most about half of them with constant probability.
    \item Average over a uniform random labeling of the shattered points.
    \item On every unseen rare point, the algorithm's expected error is $1/2$.
    \item Use the probabilistic method to select one fixed target attaining the average lower bound.
    \item A Chernoff bound supplies the constant-probability event on the number of observed rare points.
  \end{enumerate}
  \vspace{0.06cm}
  \KeyBox{The argument is information-theoretic and applies even to computationally unlimited learners.}
\end{frame}

% 70 --------------------------------------------------------------------------
\begin{frame}[t]{Lower Bound: Non-Realizable Case}
  \vspace{0.01cm}
  For $\vcdim(\Hc)=d>1$, any learning algorithm faces a distribution for which
  \begin{beamercolorbox}[rounded=true,sep=0.18cm,wd=\linewidth]{alertblock body}
    \[
      \boxed{
      \Prb_{S\sim\D^m}\!\left[
        R_\D(h_S)-\inf_{h\in\Hc}R_\D(h)
        >\sqrt{\frac{d}{320m}}
      \right]\ge\frac{1}{64}.}
    \]
  \end{beamercolorbox}
...
\begin{frame}[t]{What the Three Complexity Measures Tell Us}
  ...
  \end{center}}
  \vspace{0.06cm}
  \KeyBox{\small Chapter 3's central chain is
  \[
...
\begin{frame}[t]{References and Questions}
  ...
  \end{beamercolorbox}
  \vspace{0.08cm}
  {\scriptsize\color{midgray}Core mathematical content follows Chapter 3; diagrams, timing, checkpoints, and numerical illustrations are instructor-created.}
\end{frame}
